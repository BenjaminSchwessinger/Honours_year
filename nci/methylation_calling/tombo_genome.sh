...
##this is a script to do methylation-calling on the whole Pst-104E genome.
...
fast5_dir=$input/genome_aln_fast5.tar.gz
...
time tombo annotate_raw_with_fastqs --fast5-basedir $PBS_JOBFS_fast5 --fastq-filenames $PBS_JOBFS_fastq

#

#Re-squiggle Algorithm (Raw Data to Genome Alignment)

time tombo resquiggle $PBS_JOBFS_fast5 $PBS_JOBFS_ref --processes $threads 
...
mkdir 5mC 6mA
time tombo test_significance --fast5-basedirs $PBS_JOBFS_fast5 --alternate-bases 5mC 6mA --statistics-file-basename $name #--per-read-statistics-basename plot
...
time tombo write_most_significant_fasta --statistics-filename ${name}.5mC.tombo.stats --genome-fasta $PBS_JOBFS_ref 
time tombo write_wiggles --fast5-basedirs $PBS_JOBFS_fast5 --wiggle-basename 5mC --wiggle-types fraction --statistics-filename ${name}.5mC.tombo.stats
time tombo plot_most_significant --fast5-basedirs $PBS_JOBFS_fast5 --statistics-filename ${name}.5mC.tombo.stats

cd $PBS_JOBFS/alt_model/6mA 
time tombo write_most_significant_fasta --statistics-filename ${name}.6mA.tombo.stats --genome-fasta $PBS_JOBFS_ref
time tombo write_wiggles --fast5-basedirs $PBS_JOBFS_fast5 --wiggle-basename 6mA --wiggle-types fraction --statistics-filename ${name}.6mA.tombo.stats
time tombo plot_most_significant --fast5-basedirs $PBS_JOBFS_fast5 --statistics-filename ${name}.6mA.tombo.stats
...
time tombo test_significance --fast5-basedirs $PBS_JOBFS_fast5 --statistics-file-basename de_novo #--per-read-statistics-basename de_novo

#

#Extract sequences Surrounding Modified Positions
# output the genomic sequence surrounding locations with the largest fraction of modified reads
time tombo write_most_significant_fasta --statistics-filename de_novo.tombo.stats --genome-fasta $PBS_JOBFS_ref
time tombo write_wiggles --fast5-basedirs $PBS_JOBFS_fast5 --wiggle-basename de_novo --wiggle-types fraction --statistics-filename de_novo.tombo.stats
time tombo plot_most_significant --fast5-basedirs $PBS_JOBFS_fast5 --statistics-filename de_novo.tombo.stats
